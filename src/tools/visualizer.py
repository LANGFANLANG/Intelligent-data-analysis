...
if _cn_font_path:
    ...
    print(f"[Visualizer] 使用中文字体: {_font_name} ({_cn_font_path})")
else:
    plt.rcParams["font.family"] = "sans-serif"
    plt.rcParams["font.sans-serif"] = ["Microsoft YaHei", "SimHei", "DejaVu Sans"]
    plt.rcParams["axes.unicode_minus"] = False
    print("[Visualizer] 警告: 未找到中文字体，可能显示为方块")
...
def line_chart(x_column: str, y_column: str) -> str:
    ...
    try:
        ...
        plt.title(f"{y_column} 随 {x_column} 变化趋势", fontsize=16)
...
def bar_chart(x_column: str, y_column: str) -> str:
    ...
    agg_df = df.groupby(x_column)[y_column].mean().sort_values(ascending=False)
    if len(agg_df) == 0:
    ...
    try:
        ...
        plt.title(f"{y_column} 按 {x_column} 分布", fontsize=16)
        plt.xlabel(x_column, fontsize=13)
        plt.ylabel(y_column, fontsize=13)
        plt.xticks(rotation=45)
...
def pie_chart(column: str) -> str:
    ...
    try:
        ...
        plt.title(f"{column} 分布占比", fontsize=16)
